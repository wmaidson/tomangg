...
interface User {
    ...
    username: string;
    dkp: number;
    guildBoss: number; // Adicionada a propriedade para guildBoss
}
...
export default function GuildBoss() {
    ...
    const [bossValue, setBossValue] = useState<number>(1);
    ...
    const handleSave = async () => {
        if (selectedUsers.length === 0) {
            ...
            try {
                // Atualizar os valores de guildBoss e dkp no backend
                const updates = selectedUsers.map((user) => ({
                    id: user.id,
                    guildBoss: user.guildBoss + bossValue,
                    dkp: user.dkp + bossValue * dpkValue,
                }));
    ...
    const handleReset = async () => {
        try {
            await axios.put("http://localhost:3333/users/reset-bosses");
            await fetchUsers(); // Atualiza os dados após redefinir os bosses
            setShowResetConfirmationModal(false); // Fecha o modal de confirmação
            setShowSuccessResetModal(true); // Abre o modal de sucesso
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    Quantidade de Bosses Realizados
                </Typography>
            ...
            >
                ...
                >
                    ...
                    >
                        {/* Grupo 1: Quantidade de Boss */}
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            <Typography variant="body1">
                                Selecione a quantidade de Boss
                            </Typography>
                            <TextField
                                type="number"
                                value={bossValue}
                                onChange={(e) => setBossValue(Number(e.target.value))}
                                sx={{
                                    backgroundColor: "#3A3D45",
                                    input: { color: "#D9D3C9" },
                                    width: "80px",
            ...
            <Modal open={showSuccessModal} onClose={handleCloseSuccessModal}>
                ...
                {bossValue}
                        ...
                        Boss e receberam{" "}
                        <span style={{ color: "lightgreen", fontWeight: "bold" }}>
                {selectedUsers.length * bossValue * dpkValue} DKP
            </span>
                        .
                    </Typography>
            ...
            <Modal open={showResetModal} onClose={() => setShowResetModal(false)}>
                ...
                >
                    <Typography variant="h6" sx={{ textAlign: "center", marginBottom: 3 }}>
                        Realmente deseja redefinir todos os boss?
                    </Typography>
            ...
            >
                ...
                >
                    <Typography variant="h6" sx={{ marginBottom: 3 }}>
                        Bosses redefinidos com sucesso!
                    </Typography>
                    <Button
                        variant="contained"
